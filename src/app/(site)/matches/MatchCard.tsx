...
function formatScheduleDate(
  ...
  const tz = timeZone ?? "Europe/Helsinki";
  const d = new Date(`${date}T00:00:00Z`);
  const opts = { timeZone: tz } as const;
  const scheduledTime = time ?? "18:00";
  return {
...
export default function MatchCard({
  ...
  const { weekday, day, month, timeStr } = formatScheduleDate(
    game.date,
    game.time,
    (game as Game & { timeZone?: string }).timeZone,
  );
    ...
    return (
      ...
      >
        ...
        <span
          ...
          data-gold={label.gold || undefined}
        >
          {label.text}
        </span>
  ...
  // Compact variant
  const apSmallLogoClass = [
    styles.smallLogo,
    apData?.invertLogo && styles.invertLogo,
  ]
    .filter(Boolean)
    .join(" ");
  const oppSmallLogoClass = [
    styles.smallLogo,
    oppData?.invertLogo && styles.invertLogo,
  ]
    .filter(Boolean)
